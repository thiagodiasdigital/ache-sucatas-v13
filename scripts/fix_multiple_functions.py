"""
Script para remover versões duplicadas da função fetch_auctions_paginated
e garantir que apenas uma versão existe
"""
...
def main():
    print("=" * 60)
    print("FIX: REMOVER FUNÇÕES DUPLICADAS")
    print("=" * 60)
    ...
    try:
        ...
        # Listar todas as versões da função
        print("\n1. Listando versões existentes da função...")
        cursor.execute("""
            SELECT
        ...
        for func in functions:
            print(f"   - {func[2]}.{func[0]}({func[1][:50]}...)")

        # Remover TODAS as versões antigas
        print("\n2. Removendo todas as versões existentes...")

        drop_statements = [
            "DROP FUNCTION IF EXISTS public.fetch_auctions_paginated(TEXT, TEXT, NUMERIC, NUMERIC, DATE, DATE, DATE, DATE, INTEGER, INTEGER);",
            "DROP FUNCTION IF EXISTS public.fetch_auctions_paginated(TEXT, TEXT, NUMERIC, NUMERIC, DATE, DATE, DATE, DATE, INTEGER, INTEGER, TEXT);",
        ...
        # Recriar a função única
        print("\n3. Criando função única...")
...
"""
        ...
        # Verificar que só tem uma versão agora
        ...
        functions = cursor.fetchall()
        print(f"   Agora existem {len(functions)} versão(ões):")
        for func in functions:
        ...
        cursor.execute("""
            SELECT
                (public.fetch_auctions_paginated(
                    ...
                    1, 20, 'proximos', 'todos'
                )::json->>'total')::int as total
        """)
        result = cursor.fetchone()
        print(f"   MG (todos): {result[0]} leilões")

        # Verificar latitude/longitude no primeiro resultado
        cursor.execute("""
            SELECT
                public.fetch_auctions_paginated(
                    'MG', NULL, NULL, NULL, NULL, NULL, NULL, NULL,
                    1, 1, 'proximos', 'todos'
                )::json->'data'->0 as primeiro
        """)
        result = cursor.fetchone()
        import json
        primeiro = json.loads(result[0]) if result[0] else None
        if primeiro:
            print(f"   Primeiro leilão: {primeiro.get('cidade')}")
            print(f"   Latitude: {primeiro.get('latitude')}")
            print(f"   Longitude: {primeiro.get('longitude')}")

        cursor.close()
        conn.close()

        print("\n" + "=" * 60)
        print("SUCESSO! Função corrigida.")
        print("Recarregue o dashboard no navegador.")
        print("=" * 60)
