"""
ACHE SUCATAS MINER V13 - DATA QUALITY FIX
==========================================
Minerador de editais PNCP com busca de data_leilao da API COMPLETA.

NOVIDADES V13:
- FILTRO DE DATA PASSADA: Ignora editais com data_leilao < hoje
- VALIDAÇÃO DE DADOS: Garante que só entram leilões futuros no banco
- MÉTRICAS NOVAS: Contagem de editais filtrados por data passada
...
- Todas as configurações de Cloud, Storage e Supabase

CORREÇÃO CRÍTICA V13:
- V12 permitia leilões com data_leilao de 2024 (já encerrados)
- V13 filtra automaticamente: só processa leilões com data_leilao >= hoje

Data: 2026-01-20
Autor: Claude Code (CRAUDIO)
Baseado em: V12 (AURION PATCH)
"""
...
class Settings:
    """Global configuration constants - V13 DATA QUALITY FIX."""
    ...
    VERSAO_MINER = "V13_DATA_QUALITY"  # Version Bump para rastreio
    ...
    CHECKPOINT_FILE = ".ache_sucatas_checkpoint_v13.json"
    METRICS_FILE = "ache_sucatas_metrics_v13.jsonl"

    # =========================================================================
    # V13: FILTRO DE DATA PASSADA
    # =========================================================================
    # Ignora leilões com data_leilao anterior a hoje
...
log = logging.getLogger("AcheSucatas_V13")
...
# ==============================================================================
# METRICS TRACKER - V13 ENHANCED
# ==============================================================================

class MetricsTracker:
    """Rastreamento de métricas para análise de performance - V13 Enhanced."""
    ...
    def __init__(self, metrics_file: str = Settings.METRICS_FILE):
        ...
        self.metrics = {
            ...
            # V13: Nova métrica - editais filtrados por data passada
    ...
    def print_summary(self):
        log.info("=" * 70)
        log.info("RESUMO DA EXECUÇÃO - MINER V13 (DATA QUALITY)")
        log.info("=" * 70)
        ...
        log.info(f"  |- Filtrados (data passada): {self.metrics['editais_filtrados_data_passada']}")
        log.info("-" * 70)
        log.info("V13 - BUSCA DE DATA_LEILAO:")
...
class EditalProcessor:
    ...
    async def _process_item(self, client: PncpApiClient, item: dict):
        ...
        try:
            ...
            # ================================================================
            # V13: VALIDAÇÃO DE DATA PASSADA
            # ================================================================
            # Se data_leilao foi encontrada e está no passado, ignora o edital
            if Settings.FILTRAR_DATA_PASSADA and data_leilao:
                hoje = datetime.now().replace(hour=0, minute=0, second=0, microsecond=0)
                if data_leilao < hoje:
                    self.metrics.increment("editais_filtrados_data_passada")
                    log.debug(f"Edital {pncp_id} IGNORADO: data_leilao {data_leilao.date()} < hoje {hoje.date()}")
                    return  # Não processa este edital
...
class AcheSucatasMiner:
    ...
    async def run(self):
        """Execute mining process - V13 DATA QUALITY FIX."""
        log.info("=" * 70)
        log.info("ACHE SUCATAS MINER V13 - DATA QUALITY")
        log.info("=" * 70)
        log.info("NOVIDADE V13: Filtra leilões com data passada!")
        log.info("-" * 70)
        ...
        log.info(f"Filtrar data passada: {'ATIVO' if Settings.FILTRAR_DATA_PASSADA else 'DESATIVADO'}")
