"""
Ache Sucatas DaaS - Minerador V17
=================================
CORRECAO DO V16: Restaura API de detalhes para obter campos obrigatorios.

Versao: 17.0
Data: 2026-01-23

Changelog V17.0:
    - CORRIGIDO: Restaura API de detalhes do PNCP para obter:
        * dataAberturaProposta -> data_leilao
        * valorTotalEstimado -> valor_estimado
        * dataPublicacaoPncp -> data_publicacao

    - CORRIGIDO: Limpeza de URLs (remove ), ., , no final)

    - MANTIDO do V16 (Prompt Garantidor):
        * Mapeamento EXCLUSIVO dos campos
        * n_edital EXCLUSIVAMENTE do PDF (sem fallback)
        * Validador e roteamento para quarentena
        * QualityReport

Baseado em: V16
Autor: Claude (Correcao API Detalhes)
Contrato: contracts/dataset_contract_v1.md

===============================================================
REGRA: Se o campo nao existir na API, o registro vai para
       quarentena. Nao inventamos dados.
===============================================================
"""
...
def extrair_texto_pdf(pdf_bytes: bytes) -> str:
    ...
    try:
        ...
        logging.getLogger("MinerV17").warning("pypdfium2 nao instalado - extracao de PDF desabilitada")
        return ""
    except Exception as e:
        logging.getLogger("MinerV17").debug(f"Erro ao extrair texto do PDF: {e}")
        return ""
...
@dataclass
class MinerConfig:
    """Configuracoes do minerador V17."""
...
logger = logging.getLogger("MinerV17")
...
# ============================================================
# MINERADOR PRINCIPAL V17
# ============================================================

class MinerV17:
    """
    Minerador de editais do PNCP - Versao 17.

    CORRECAO V17:
    - Restaura API de detalhes para obter campos obrigatorios
    - Mapeamento EXCLUSIVO mantido (Prompt Garantidor)
    - Limpeza de URLs corrigida
    """
    ...
    def __init__(self, config: MinerConfig):
        ...
        self.logger = logging.getLogger("MinerV17")
    ...
    def _processar_edital(self, item: dict) -> bool:
        ...
        V17: Usa busca + API detalhes + PDF.
        """
        pncp_id = item.get("numeroControlePNCP") or item.get("numero_controle_pncp") or item.get("pncp_id")
    ...
    def executar(self) -> dict:
        ...
        self.logger.info("=" * 70)
        self.logger.info("ACHE SUCATAS MINER V17 - API DETALHES RESTAURADA")
        self.logger.info("=" * 70)
        ...
        self.logger.info("-" * 70)
        self.logger.info("V17 CORRECOES:")
        self.logger.info("  - API detalhes RESTAURADA para obter campos obrigatorios")
        self.logger.info("  - data_leilao      <- dataAberturaProposta (API detalhes)")
        self.logger.info("  - valor_estimado   <- valorTotalEstimado (API detalhes)")
        self.logger.info("  - data_publicacao  <- dataPublicacaoPncp (API detalhes)")
        self.logger.info("  - n_edital         <- EXCLUSIVAMENTE do PDF")
        self.logger.info("  - URLs limpas (remove caracteres invalidos)")
        self.logger.info("=" * 70)
    ...
    def _imprimir_resumo(self):
        ...
        self.logger.info("=" * 70)
        self.logger.info("RESUMO DA EXECUCAO - MINER V17")
        self.logger.info("=" * 70)
...
def main():
    """Ponto de entrada do minerador V17."""
    parser = argparse.ArgumentParser(
        description="Ache Sucatas Miner V17 - Minerador de editais PNCP (API Detalhes Restaurada)"
    )
    parser.add_argument(
        "--dias",
        type=int,
