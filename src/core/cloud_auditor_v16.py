...
"""
ACHE SUCATAS DaaS - CLOUD AUDITOR V16
=====================================
Extrai dados de editais usando cascata de fontes - 100% CLOUD.
COM NORMALIZAÇÃO DE MODALIDADES E TAGS!
...
- CORREÇÃO DE MODALIDADE: Se título diz "Online" mas modalidade é "Presencial", corrige
...
- COMPATÍVEL: Processa editais de V10, V11, V12, V13
- RATE LIMITING: Delay configurável entre chamadas à API

CORREÇÕES CRÍTICAS V16:
- V15 criava tags "sync" e "leilao" (redundantes)
- V15 não normalizava modalidades (7 variações diferentes)
- V15 não corrigia modalidade quando título contradizia
...
log = logging.getLogger("CloudAuditor_V16")
...
class Settings:
    """Cloud Auditor V16 configuration."""
    ...
    VERSAO_AUDITOR = "V16_DATA_QUALITY"

    # V16: Tags a serem removidas (não devem existir)
    ...
    CSV_OUTPUT = Path(os.getenv("CSV_OUTPUT", "analise_editais_v16.csv"))


# ==============================================================================
# METRICS TRACKER - V16 DATA QUALITY
# ==============================================================================

@dataclass
class AuditorMetrics:
    """Métricas detalhadas do Auditor V16."""
    ...
    def print_summary(self):
        ...
        log.info("=" * 70)
        log.info("RESUMO - CLOUD AUDITOR V16 (DATA QUALITY)")
        log.info("=" * 70)
...
def extrair_tags_inteligente(descricao: str, pdf_text: str, titulo: str) -> str:
    """
    Extrai tags do edital - V16: Não cria tags 'sync' e 'leilao'.
    """
    ...
    # V16: Se não encontrou nada útil, adiciona 'veiculo' se mencionar veículo
    # MAS NÃO adiciona 'leilao' (redundante - todo edital é de leilão)
    if not tags_encontradas:
        if 'veículo' in texto_completo or 'veiculo' in texto_completo:
            tags_encontradas.add('veiculo')
        # REMOVIDO V16: Não adicionar tag 'leilao' - é redundante
        # if 'leilão' in texto_completo or 'leilao' in texto_completo:
        #     tags_encontradas.add('leilao')

    # V16: Remover tags proibidas
    tags_encontradas = tags_encontradas - Settings.TAGS_PROIBIDAS
...
def normalizar_modalidade(modalidade: str, titulo: str = "", descricao: str = "") -> str:
    """
    V16: Normaliza modalidade para um dos 3 valores padrão.

    Args:
        modalidade: Modalidade atual (pode ter variações)
        titulo: Título do edital (para detectar contradições)
        descricao: Descrição do edital (para detectar contradições)

    Returns:
        Modalidade normalizada: "Eletrônico", "Presencial" ou "Híbrido"
    """
    ...
    # Depois, verifica se título/descrição contradiz a modalidade
    texto = f"{titulo} {descricao}".lower()

    # Se modalidade é "Presencial" mas texto menciona "online", corrige para Eletrônico ou Híbrido
    if modalidade_normalizada == "Presencial":
        if "online" in texto or "eletrônico" in texto or "eletronico" in texto:
...
def limpar_tags(tags: list) -> list:
    """
    V16: Remove tags proibidas (sync, leilao) de uma lista de tags.
    """
    if not tags:
        return tags
...
# ==============================================================================
# CLOUD AUDITOR V16 CLASS
# ==============================================================================

class CloudAuditor:
    """
    Auditor V16 que processa editais do Supabase Storage.

    V16 - DATA QUALITY:
    - Normalização de modalidades (Eletrônico, Presencial, Híbrido)
    - Limpeza de tags proibidas (sync, leilao)
    - Detecção de contradições (título "Online" vs modalidade "Presencial")

    V15 - Mantido:
    - Fallback para API PNCP quando extração do PDF falhar
    """
    ...
    def listar_editais_pendentes(self, limit: int = None) -> List[dict]:
        """Lista editais que precisam ser processados pelo Auditor V16."""
        if not self.supabase_repo or not self.supabase_repo.enable_supabase:
            return []

        try:
            # V16: Processa editais de V10, V11, V12, V13, V14, V15
            query = (
                ...
                .or_(
                    ...
                    "versao_auditor.eq.V13_DATA_QUALITY,"
                    "versao_auditor.eq.V14_CLOUD,"
                    "versao_auditor.eq.V15_API_FALLBACK"
                )
    ...
    def processar_edital(self, edital: dict) -> Optional[dict]:
        ...
        if pdf_text:
            ...
            # Modalidade - V16: Sempre normaliza
            ...
            if not modalidade_atual or modalidade_atual == "N/D":
                ...
                if modalidade != "N/D":
                    # V16: Normaliza a modalidade extraída
                ...
                # V16: Mesmo se já existe, normaliza e verifica contradições
                ...
                if modalidade_normalizada and modalidade_normalizada != modalidade_atual:
                    ...
                    log.debug(f"Modalidade normalizada: {modalidade_atual} -> {modalidade_normalizada}")

            # Tags - V16: Limpa tags proibidas
            ...
            if not tags_atuais or tags_atuais in [["miner_v10"], ["miner_v11"], ["miner_v12"], ["miner_v13"]]:
                ...
                # V16: Mesmo se já existe, limpa tags proibidas
                tags_limpas = limpar_tags(tags_atuais)
                if tags_limpas != tags_atuais:
                    dados_extraidos["tags"] = tags_limpas
                    log.debug(f"Tags limpas: {tags_atuais} -> {tags_limpas}")

        # Marcar como processado pelo V16
    ...
    def run(
        ...
        """
        log.info("=" * 70)
        log.info("ACHE SUCATAS - CLOUD AUDITOR V16 (DATA QUALITY)")
        log.info("=" * 70)
        log.info("NOVIDADE V16: Normaliza modalidades e limpa tags!")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="ACHE SUCATAS - Cloud Auditor V16 (Data Quality + API Fallback)")
